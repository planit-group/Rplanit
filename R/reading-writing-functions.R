...
#' 
#' La trasformazione in dataframe viene fatta a "splice" per risparmiare memoria
#' 
#' OBSOLETO
#' 
#' @family R/W Arrays
...
read.3d.spliced <- function(file.name,
                            x.min=-Inf, x.max=+Inf,
                            y.min=-Inf, y.max=+Inf,
                            z.min=-Inf, z.max=+Inf,
                            Nsplices=16) {
  ...
  # seleziona sottoarray (non funziona se i.min=i.max e simili, in questo caso usare read.3d()
  Values.3d <- Values.3d[ 1:Nv, i.min:i.max, j.min:j.max, k.min:k.max]
  ...
  for(i in 1:Nsplices) {
    ...
    # seleziona sottoarray
    if(Nv>1){
      Values.3d.s <- Values.3d[ , 1:Nx, 1:Ny, k.min:k.max]
    } else {
      Values.3d.s <- Values.3d[1:Nx, 1:Ny, k.min:k.max]
    }
    ...
    # melting
    cat('melting...\n')
    values.s <- melt(Values.3d.s)
    if(Nv==1) {
      values.s$X3 <- values.s$X3 + k.min-1
    } else {
      values.s$X4 <- values.s$X4 + k.min-1
    }
    #print(summary(Values.s))
   
    # assigning variables
    cat('assigning variables...\n')
    if (Nv==1) {
      values.s <- cbind(1, values.s)
    }
    colnames(values.s) <- c('variable', 'x', 'y', 'z', 'value')
    values.s$variable <- as.factor(values.s$variable)
    levels(values.s$variable) <- variables
    values.s$x <- (sapply(values.s$x, function(v) x[v]))
    values.s$y <- (sapply(values.s$y, function(v) y[v]))
    values.s$z <- (sapply(values.s$z, function(v) z[v]))

    #print(range(values.s$z)) 
...
#' Legge array da un file "3d" e restituisce un dataframe
#' 
#' OBSOLETO
#' 
#' @family R/W Arrays
#' @importFrom reshape2 melt
read.3d.df <- function(file.name,
                    x.min=-Inf, x.max=+Inf,
                    y.min=-Inf, y.max=+Inf,
                    z.min=-Inf, z.max=+Inf) {
  ###############
  ...
  if(values.format=='BINARY') {
    ...
    cat('reading ascii data...\n')
    myvector <- as.numeric(unlist(strsplit(readLines(file.3d), ' +')))
    #print(myvector)
    Values.3d <- array(myvector, dim=c(Nv, Nx, Ny, Nz))
  }
  ...
  # seleziona sottoarray (non funziona se i.min=i.max)
  if (i.min<i.max & j.min<j.max & k.min<k.max) {
    Values.3d <- Values.3d[ , i.min:i.max, j.min:j.max, k.min:k.max]
    skipped <- FALSE # la struttura dimensionale dell'array e' potenzialmente compromessa sulle variables...
  } else {
    cat('skipping subvolume selection (degenerate dimensions)...\n')
    skipped <- TRUE # serve per indicare che la struttura dimensionale dell'array e' intatta.
  }
  ...
  Values <- melt(Values.3d)
  rm(Values.3d)
  if (Nv==1 & !skipped) {
    Values <- cbind(1, Values)
  }
  names(Values) <- c('variable', 'x', 'y', 'z', 'value')
...
#' 
#' @param file.name the file name
#' @param melted return a "melted" dataframe (optional)
#' @return a dataframe
#' 
#' @family R/W Arrays
#' @export
#' @importFrom reshape2 melt
read.1d <- function(file.name, melted=FALSE) {
  ...
  # melting
  if(melted) {
    if(x.type=='INTERVAL') {id.vars <- c(1,2,3)} else {id.vars <- 1}
    values <- melt(values, id.vars=id.vars)
  }
